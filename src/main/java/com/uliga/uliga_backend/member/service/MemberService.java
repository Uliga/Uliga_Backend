package com.uliga.uliga_backend.member.service;
...
public class MemberService {
  ...
  // NotFoundByIdException("해당 아이디로 존재하는 멤버가 없습니다"));
  ...
  // // List<InvitationInfo> invitationInfos = result.subList((int)
  // // pageable.getOffset(), pageable.getPageSize());
  // // new PageImpl<>(invitationInfos, pageable, result.size());
  // return GetMemberInfo.builder().memberInfo(memberInfoById)

  // .invitations(result).notifications(notificationInfos).build();
  // }
  ...
  // Member member = memberRepository.findById(id).orElseThrow(() -> new
  // NotFoundByIdException("해당 아이디로 존재하는 멤버가 없습니다"));
  // return passwordEncoder.matches(passwordCheck.getApplicationPassword(),
  // member.getApplicationPassword());
  // }
  ...
  // return passwordEncoder.matches(passwordCheck.getPassword(),
  // member.getPassword());
  ...
  // } else {
  // return
  // memberRepository.existsByNickNameAndDeleted(nicknameCheckDto.getNickname(),
  // false);
  // }
  ...
  // */
  // @Transactional(readOnly = true)
  // public SearchEmailResult findMemberByEmail(Long accountBookId,
  // SearchMemberByEmail byEmail) {
  // Member member = memberRepository.findByEmailAndDeleted(byEmail.getEmail(),
  // false).orElseThrow(UserNotFoundByEmail::new);
  // if (accountBookId != null) {

  // * if (
  // ccountBookMemberRepository.existsAccountBookMemberByMemberIdAndAccountBookId(member.getId(),
  // accountBookId)) {
  ...
  // */
  // @Transactional
  // public MemberInfoUpdateRequest updateMemberInfo(Long id, Map<String, Object>
  // updates) {
  // MemberInfoUpdateRequest memberInfoUpdateRequest =
  // objectMapper.convertValue(updates, MemberInfoUpdateRequest.class);
  // Member member = memberRepository.findById(id).orElseThrow(() -> new
  // NotFoundByIdException("해당 아이디로 존재하는 멤버가 없습니다"));
  // if (memberInfoUpdateRequest.getApplicationPassword() != null) {
  // String encode =
  // passwordEncoder.encode(memberInfoUpdateRequest.getApplicationPassword());
  // member.updateApplicationPassword(encode);

  // * }
  // if (memberInfoUpdateRequest.getPassword() != null) {
  // String encode =
  // passwordEncoder.encode(memberInfoUpdateRequest.getPassword());
  ...
  // deleteMemberNotification(Long id) {
  // SetOperations< tring, Object> setOperations =
  ...
  // accountBookMemberRepository.deleteAccountBookMemberByAccountBookIdAndMemberId(accountBookId,
  // memberId);
  // return "DELETED";
  // }
}
