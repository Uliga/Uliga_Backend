...
public class AccountBookService {
  ...
  // AccountBookInfoQ bookInfoById =
  // accountBookRepository.findAccountBookInfoById(id, memberId);
  ...
  // List<AccountBookInfoQ> accountBookInfosByMemberId =
  // accountBookRepository.findAccountBookInfosByMemberId(id);
  ...
  // .numberOfMember(
  // accountBookRepository.getMemberNumberByAccountBookId(accountBookInfoQ.getAccountBookId()))
  // .categories(categoryRepository.findAccountBookCategoryInfoById(accountBookInfoQ.getAccountBookId()))
  // .build();
  // result.add(build);
  // }
  ...
  // */
  // @Transactional
  // public AccountBook createAccountBookPrivate(Member member,
  // CreateRequestPrivate createRequest) {
  ...
  // */
  // @Transactional
  // public AccountBook createAccountBookPrivateSocialLogin(Long memberId,
  // CreateRequestPrivate createRequest) {
  // AccountBook accountBook = createRequest.toEntity();
  // accountBookRepository.save(accountBook);
  // Member member =
  ...
  // */
  // @Transactional
  // public AccountBook createAccountBook(Long id, AccountBookCreateRequest
  ...
  // InvitationInfo info = new InvitationInfo(accountBook.getId(),
  // member.getUserName(), accountBook.getName());
  ...
  // */
  // @Transactional
  // public Invited createInvitation(Long id, GetInvitations invitations) throws
  // JsonProcessingException {
  // Member member = memberRepository.findById(id)
  // .orElseThrow(() -> new NotFoundByIdException("해당 아이디로 존재하는 멤버가 없습니다"));
  // AccountBook accountBook = accountBookRepository.findById(invitations.getId())
  // .orElseThrow(() -> new NotFoundByIdException("가계부 초대에 아이디 값이 없습니다"));
  // long result = 0L;
  ...
  // */
  // @Transactional
  // public InvitationReplyResult invitationReply(Long id, InvitationReply
  // invitationReply)
  // throws JsonProcessingException {
  // Member member = memberRepository.findById(id)
  // .orElseThrow(() -> new NotFoundByIdException("해당 아이디로 존재하는 멤버가 없습니다"));
  ...
  // if
  // (!accountBookMemberRepository.existsAccountBookMemberByMemberIdAndAccountBookId(member.getId(),
  // invitationReply.getId())) {
  // AccountBook accountBook =
  // accountBookRepository.findById(invitationReply.getId())
  ...
  // .build();
  // SetOperations<String, Object> setOperations = redisTemplate.opsForSet();
  // setOperations.remove(member.getEmail(),
  // objectMapper.writeValueAsString(build));
  // return new InvitationReplyResult(invitationReply.getId(),
  // invitationReply.getJoin());
  ...
  // return new
  // AccountBookMembers(accountBookRepository.findAccountBookMemberInfoById(id));
  // }
  ...
  // .orElseThrow(() -> new NotFoundByIdException("해당 아이디로 존재하는 가계부가 없습니다"));
  // if
  // (accountBookMemberRepository.existsAccountBookMemberByMemberIdAndAccountBookId(memberId,
  // id)) {
  ...
  // */
  // @Transactional
  // public AccountBookUpdateRequest updateAccountBookInfo(Long memberId, Long
  // accountBookId,
  // AccountBookUpdateRequest request) throws JsonProcessingException {
  // AccountBook accountBook = accountBookRepository.findById(accountBookId)
  // .orElseThrow(() -> new NotFoundByIdException("해당 아이디로 존재하는 가계부가 없습니다"));
  // AccountBookMember accountBookMember = accountBookMemberRepository
  // .findAccountBookMemberByMemberIdAndAccountBookId(memberId, accountBookId)
  // .orElseThrow(UnauthorizedAccountBookAccessException::new);
  // if (request.getAvatarUrl() != null) {
  // accountBookMember.setAvatarUrl(request.getAvatarUrl());
  // }

  // if (request.getMembers() != null) {
  // createInvitation(memberId,
  // GetInvitations.builder().id(accountBookId).emails(request.getMembers()).build());
  // }
