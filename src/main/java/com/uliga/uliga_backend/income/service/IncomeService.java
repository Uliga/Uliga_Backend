package com.uliga.uliga_backend.income.service;
...
public class IncomeService {
  ...
  // public MonthlySumQ getMonthlyIncomeSum(Long accountBookId, Long year, Long
  // month) {
  // return incomeRepository.getMonthlySumByAccountBookId(accountBookId, year,
  // month).orElse(new MonthlySumQ(0L));
  // }

  // @Transactional

  // public AddIncomeResult addIncome(Long currentMemberId, AddIncomeRequest
  // addIncomeRequest) {

  // AccountBook accountBook = accountBookRepository.findById(a
  // ddIncomeRequest.getId()).orElseThrow(() -> new NotFoundByIdException("해당 아이디로
  // 존재하는 가계부가 없습니다"));
  // Category category = categoryRepository.findByAccountBookAndName(accountBook,
  // addIncomeRequest.getCategory()).orElseThrow(CategoryNotFoundException::new);
  // Member member = memberRepository.findById(currentMemberId).orElseThrow(() ->
  // new NotFoundByIdException("해당 아이디로 존재하는 멤버가 없습니다"));
  ...
  // List<AccountBook> sharedAccountBooks =
  // accountBookRepository.findAccountBookByAccountBookIds(sharedAccountBookIds);
  ...
  // IncomeUpdateRequest patchIncome = objectMapper.convertValue(updates,
  // IncomeUpdateRequest.class);
  // if (patchIncome.getId() == null) {
  // throw new IdNotFoundException("수입 아이디가 null 입니다.");

  // }
  // Income income = incomeRepository.findById(patchIncome.getId()).orElseThrow(()
  // -> new NotFoundByIdException("해당 아이디로 존재하는 수입이 없습니다"));
  ...
  // Category category =
  // categoryRepository.findByAccountBookAndName(income.getAccountBook(),
  // patchIncome.getCategory()).orElseThrow(CategoryNotFoundException::new);
  ...
  // }
  // if (patchIncome.getType() != null &&
  // patchIncome.getType().equals(AccountBookDataType.RECORD.toString())) {
  // income.updateType();
  ...
  // @Transactional(readOnly = true)
  // public Page<IncomeInfoQ> getMemberIncomesByAccountBook(Long accountBookId,
  // Long categoryId, Long year, Long month, Pageable pageable) {
  ...
  // List<IncomeInfoQ> accountBookMemberIncomes =
  // incomeMapper.findAccountBookMemberIncomes(map);
  ...
  // Income income = incomeRepository.findById(incomeId).orElseThrow(() -> new
  // NotFoundByIdException("해당 아이디로 존재하는 수입이 없습니다"));
