package com.uliga.uliga_backend.budget.service;
...
public class BudgetService {
  ...
  // public MonthlySumQ getMonthlyBudgetSum(Long accountBookId, Long year, Long
  // month) {
  // return budgetRepository.getMonthlySumByAccountBookId(accountBookId, year,
  ...
  // CreateBudgetDto createBudgetDto = mapper.convertValue(createBudgetMap,
  // CreateBudgetDto.class);
  // AccountBook accountBook =
  // accountBookRepository.findById(createBudgetDto.getId()).orElseThrow(() -> new
  // NotFoundByIdException("해당 아이디로 존재하는 가계부가 없습니다"));
  // Optional<Budget> budgetByAccountBookIdAndYearAndMonth =
  // budgetRepository.findByAccountBookIdAndYearAndMonth(createBudgetDto.getId(),
  // createBudgetDto.getYear(), createBudgetDto.getMonth());
  ...
  // createBudgetDto.getCategory()).orElseThrow(CategoryNotFoundException::new);
  ...
  // */
  // @Transactional(readOnly = true)
  // public BudgetCompare compareWithBudget(Long accountBookId, Long year, Long
  // month) {
  // Optional<MonthlySumQ> recordSum =
  // recordRepository.getMonthlySumByAccountBookId(accountBookId, year, month);
  // Optional<MonthlySumQ> budgetSum =
  // budgetRepository.getMonthlySumByAccountBookId(accountBookId, year, month);
  // if (recordSum.isPresent() && budgetSum.isPresent()) {
  // MonthlySumQ record = recordSum.get();
  // MonthlySumQ budget = budgetSum.get();
  // return new BudgetCompare(budget.getValue(), record.getValue(),
  // budget.getValue() - record.getValue());
  ...
  // BudgetUpdateRequest updateRequest = mapper.convertValue(updates,
  ...
  // }
  // Budget budget =
  // budgetRepository.findByAccountBookIdAndYearAndMonth(updateRequest.getId(),
  // updateRequest.getYear(),
  // updateRequest.getMonth()).orElseThrow(BudgetNotExistsException::new);
  ...
  // Category category =
  // categoryRepository.findByAccountBookIdAndName(updateRequest.getId(),
  // updateRequest.getCategory()).orElseThrow(CategoryNotFoundException::new);
  // budget.updateCategory(category);
  // }
